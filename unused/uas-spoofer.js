...
var DONT_SPOOF_JS_OBJECTS = "dont_spoof_js_objects";
...
var ERROR_CODES = [TEST_ERROR_CODE, OUT_OF_SPACE, MIGRATION_INCOMPLETE, MIGRATION_FAILED_THROTTLED_USER_AGENTS, MIGRATION_FAILED_THROTTLED_SPOOFS, THROTTLED_WRITES];
...
function _isUsingLocalStorage() {
    return ( localStorage[CUSTOM_OPTIONS_IDENTIFIER] > 0 || localStorage[SPOOF_OPTIONS_IDENTIFIER] > 0) ;
}
...
function setDoNotSpoofJSObjects(dont_spoof_js) {
    getIsOtherSettingsManaged(function(managed) {
        if (managed == null  || !managed)
            storeItem(DONT_SPOOF_JS_OBJECTS, "" + dont_spoof_js);
    });
}
function getDoNotSpoofJSObjects(callback) {
    getIsOtherSettingsManaged(function(managed) {
        if (managed == null  || !managed) {
            var dont_spoof_js = getItem(DONT_SPOOF_JS_OBJECTS);
            dont_spoof_js = (dont_spoof_js && dont_spoof_js == "true");
        } else
            dont_spoof_js = false;
        callback(dont_spoof_js);
    });
}
function getIsUserAgentManaged(callback) {
    chrome.storage.managed.get(MANAGED_EDIT_RIGHTS, function(edit_rights) {
        callback(edit_rights && MANAGED_EDIT_RIGHTS in edit_rights && "user_agents" in edit_rights[MANAGED_EDIT_RIGHTS]);
    });
}
function getIsSpoofManaged(callback) {
    chrome.storage.managed.get(MANAGED_EDIT_RIGHTS, function(edit_rights) {
        callback((edit_rights && MANAGED_EDIT_RIGHTS in edit_rights && "permanent_spoofs" in edit_rights[MANAGED_EDIT_RIGHTS]));
    });
}
function getIsOtherSettingsManaged(callback) {
    chrome.storage.managed.get(MANAGED_EDIT_RIGHTS, function(edit_rights) {
        callback(edit_rights && MANAGED_EDIT_RIGHTS in edit_rights && "other_settings" in edit_rights[MANAGED_EDIT_RIGHTS]);
    });
}
function getSpoofOverrideManaged(callback) {
    chrome.storage.managed.get("OtherSettings", function(other_settings) {
        if (other_settings && other_settings.OtherSettings && "spoof_override" in other_settings.OtherSettings)
...
function getHotlistEnabledManaged(callback) {
    chrome.storage.managed.get("OtherSettings", function(other_settings) {
        if (other_settings && other_settings.OtherSettings && "hotlist_enabled" in other_settings.OtherSettings)
...
function getSpoofPerTabManaged(callback) {
    chrome.storage.managed.get("OtherSettings", function(other_settings) {
        if (other_settings && other_settings.OtherSettings && "spoof_per_tab" in other_settings.OtherSettings)
...
function getSendErrorsManaged(callback) {
    chrome.storage.managed.get("OtherSettings", function(other_settings) {
        if (other_settings && other_settings.OtherSettings && "send_errors" in other_settings.OtherSettings)
                ...
                var set_user_agent = function(list, pointer_list, callback) {
                    ...
                    getStorageLocation().set(values, function() {
                        if (chrome.runtime.lastError) {
                            ...
                            pointer_list.push("" + foo);
                            set_user_agent(list, pointer_list, callback);
                        }
                    });
                }
                ;
    ...
    getIsUserAgentManaged(function(managed) {
        if (!managed)
            _getUserAgentPointerList(function(pointer_list) {
                ...
                getStorageLocation().set({
                    CUSTOM_OPTIONS_IDENTIFIER: pointer_list
                }, function() {
    ...
    getIsSpoofManaged(function(managed) {
        if (!managed) {
            _getSpoofPointerList(function(pointer_list) {
                ...
                getStorageLocation().set({
                    SPOOF_OPTIONS_IDENTIFIER: pointer_list
                }, function() {
                    getStorageLocation().remove(key, function() {
                        updateCache(callback);
                    });
    ...
    _getUserAgentList(function(user_agent_list) {
        ...
        for (var i = 0; i < user_agent_list.length; i++) {
            if (!user_agent_list[i])
                continue;var group = getUserAgentGroup(user_agent_list[i]);
            if (!groups[group])
...
function isDomainName(str) {
    var pattern = new RegExp('^(https?:\/\/)?' + '((([a-z\d]([a-z\d-]*[a-z\d])*)\.)+[a-z]{2,}|' + '((\d{1,3}\.){3}\d{1,3}))' + '(\:\d+)?(\/[-a-z\d%_.~+]*)*$','i');
...
function guessUserAgentGroup(str) {
    if (!str)
        return "";
    var guesses = [["Chrome", "Chrome"], ["Firefox", "Firefox"], ["Opera", "Opera"], ["Safari", "Safari"], ["IE", "Internet Explorer"], ["Internet Explorer", "Internet Explorer"], ["iPhone", "iPhone"], ["iPad", "iPad"], ["iOS", "iOS"]];
        ...
        if (user_agents && user_agents.length > 0)
            _getUserAgentList(function(options) {
                ...
                for (var j = 0; j < user_agents.length; j++)
                    if (user_agents[j].title && user_agents[j].ua_string && user_agents[j].badge) {
                        var ua = new UserAgent(user_agents[j].title,user_agents[j].ua_string,user_agents[j].vendor,user_agents[j].badge,false,user_agents[j].group);
        ...
        if (permanent_spoofs && permanent_spoofs.length > 0)
            _getSpoofList(function(spoofs) {
                ...
                for (var j = 0; j < permanent_spoofs.length; j++)
                    if (permanent_spoofs[j].domain && permanent_spoofs[j].user_agent && permanent_spoofs[j].user_agent.title && permanent_spoofs[j].user_agent.ua_string && permanent_spoofs[j].user_agent.vendor && permanent_spoofs[j].user_agent.badge) {
                        var ua = new UserAgent(permanent_spoofs[j].user_agent.title,permanent_spoofs[j].user_agent.ua_string,permanent_spoofs[j].user_agent.vendor,permanent_spoofs[j].user_agent.badge,false,permanent_spoofs[j].group);
...
function importUserAgentData(raw_data, callback) {
    _getUserAgentList(function(options) {
        ...
        $('useragent', raw_data).each(function() {
            var ua = new UserAgent($(this).attr("description"),$(this).attr("useragent"),"","X",false,_getParentFolderName($(this).parent()));
...
function _reportErrors() {
    ...
    ;
    var error_string = "cv=" + _getChromeVersion() + "&ev=" + chrome.app.getDetails().version + "&";
    var errors = getErrors();
    var count = 0;
...
function _isApplicableMatch(match_pattern, url) {
    ...
    return ( (!pattern_is_domain && pattern.test(url)) || pattern.test(domain) || match_pattern == domain || match_pattern == url || (pattern_is_domain && domain.indexOf(match_pattern) > -1)) ;
}
...
function getCacheSpoofValues(url, tabId) {
    ...
    var hotlist = null ;
    for (var i = 0; (i < user_agent_cache.length && !hotlist)
        ; i++)
    ...
    function getPlatformSpoofValues(user_agent) {
        for (var key in PLATFORM_SPOOF)
            if (user_agent && user_agent.ua_string && user_agent.ua_string.indexOf(key) > -1)
                return PLATFORM_SPOOF[key];
        return null ;
    }
    function getAppnameSpoofValues(user_agent) {
        if (user_agent && user_agent.ua_string && user_agent.ua_string.indexOf("MSIE") > -1)
